...
import de.demmer.dennis.model.fileutil.MultipartConverter;
import de.demmer.dennis.model.fileutil.pdf.PdfReader;
...
@Controller
public class BuzzwordPdfController {
    ...
    @PostMapping(value = "/buzzword/pdf")
    public String uploadPDF(@RequestParam(value = "number", required = false) String number, @RequestParam(value = "language", required = false) String language, @RequestParam("file") MultipartFile multiFile, Model model) {

        try {
            String text = PdfReader.toString(MultipartConverter.toFile(multiFile));
